const selectSVG = id => {
  ...
  let textCenter = 0;
  const letters = [];
  const prompt = ['a', 'n', 'i', 'm', 'a', 't', 'i', 'o', 'n', 's', '!'];
  let runPrompt = true;
...
//   TweenLite.to(icon, 5, {opacity: 0, y: 50});
  ...
  TweenLite.to(icon, 1, {y:0, delay:10});
  ...
  TweenLite.fromTo(langss, 2, {x:-200, ease:Power2.easeOut, delay: 11, x:0, ease:Power2.easeOut, delay: 13})